...
這個小垃圾是我為了解決安卓手機不能跟macbook傳airdrop所生的，目前用起來真的挺好用。原本傳太大的檔案或影片會超時，現在也已經搞定處理好了；另外長時間掛機或 Mac 休眠後掃 QR 碼進不去的問題，也全面加上了通道健康自動守護（Auto-Healing）、休眠喚醒重連與動態刷新，照片跟影片都能順順秒傳！
...
* 🔄 **連線通道守護與自我修復 (Tunnel Auto-Healing)**
  * **主動健康探針**：內建背景 Supervisor 每 25 秒主動檢測連線活性，遇連線中斷自動秒級重連。
  * **Mac 休眠喚醒自癒**：筆電闔蓋休眠或喚醒時自動重啟加密通道，徹底告別通道失效問題。
  * **動態 QR Code & 30 分鐘長效 Token**：點擊隨時刷新最新連線憑證，杜絕過期阻擋。
  * **同 Wi-Fi 區網極速備援**：支援內網 IP 直連模式，同一 Wi-Fi 下傳輸更自由。
* 🔒 **真正的端對端直連加密 (E2EE)**
